...
module.exports.getGoodList=function (whereObj,skip,limit,cb){
  _connect(function(db){
    db.collection("goodList").aggregate([
      ...
      {
        $lookup:{
          from:"goodTypeList",// 你要和哪个集合进行合并
          localField:"goodId",// 本集合当中连接的依据字段
          foreignField:"_id",// 外部的集合连接的邻居字段
          as: 'goodInfo'// 查找到的信息放到该字段 当中
        }
      },
      {
        $lookup:{
          ...
          foreignField:"_id",// 外部的集合连接的邻居字段
          as: 'goodInfoTwo'// 查找到的信息放到该字段 当中
        }
      }
    ]).toArray(cb)
  })
}
module.exports.getGoodTypeList=function (whereObj,skip,limit,cb){
  _connect(function(db){
    db.collection("goodTypeList").aggregate([
      {
        $match:whereObj
      },
      {$sort:{addTime:-1}},
      ...
      },
      {
        $lookup:{
          from:"shopList",// 你要和哪个集合进行合并
          localField:"goodTypeId",// 本集合当中连接的依据字段
          foreignField:"_id",// 外部的集合连接的邻居字段
          as: 'goodInfo'// 查找到的信息放到该字段 当中
        }
      }
